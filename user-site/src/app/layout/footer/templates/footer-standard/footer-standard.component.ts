...
/**
 * Standard footer template - logo, column links, social links.
 * Most commonly used footer style.
 */
@Component({
    ...
    <footer class="bg-gray-900 text-white">
      <div class="mx-auto px-4 sm:px-6 lg:px-8 py-12" [style.max-width.px]="maxWidth()">
        ...
        <div class="grid gap-8" [ngClass]="{
          ...
          <div>
            @if (logoUrl()) {
              <img [src]="logoUrl()" [alt]="logoAlt()" class="h-10 mb-4 brightness-0 invert" [style.max-height.px]="maxLogoHeight()" />
            }
            @if (showSiteName()) {
              <div class="text-xl font-bold mb-4">{{ siteName() }}</div>
            }
            @if (tagline()) {
              <p class="text-gray-400 text-sm mb-4">{{ tagline() }}</p>
            }

            @if (showSocialLinks()) {
              <div class="flex items-center gap-4">
                @for (social of socialLinks(); track social.platform) {
                  <a [href]="social.url" class="text-gray-400 hover:text-white transition-colors" [attr.aria-label]="social.platform">
